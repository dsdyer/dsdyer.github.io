...
function truncate(question) {
    const truncd = Object.assign({}, question);
    if (truncd.prompt.length > 120) {
        truncd.prompt = truncd.prompt.substring(0, 120).concat('-----', truncd.prompt.substring(120))
    }
    truncd.answers = truncd.answers.map(x => {
        if (x.value.length > 75) {
            x.value = x.value.substring(0, 75).concat('-----', x.value.substring(75));
        }
        return x;
    });
    return truncd;
...
const questions = arr.map(x => new Question(x)).slice(0, -1);
const tooBig = questions.filter((x, i, a) => {
    // console.log(i, ': ', x);
    // console.log('x.prompt.length: ', x.prompt.length);
    // console.log('x.answers.some(x => x.value.length > 75): ', x.answers.some(x => x.value.length > 75));
    // console.log('(x.prompt.length > 120) || x.answers.some(x => x.value.length > 75): ', (x.prompt.length > 120) || x.answers.some(x => x.value.length > 75));
    return (x.prompt.length > 120) || x.answers.some(x => x.value.length > 75);
});
...
function sublist(items) {
    // const sublistHolder = document.createElement('li');
    const sublist = document.createElement('ul');
    items.forEach(x => {
        sublist.appendChild(listItem(x));
    });
...
function renderTruncated(text) {
    const [keep, cut] = text.split('-----');
    const span1 = document.createElement('span');
    const content1 = document.createTextNode(keep);

    span1.appendChild(content1);
    if (cut) {
        const span2 = document.createElement('span');
        const content2 = document.createTextNode(cut);
...
tooBig.forEach((x, i, a) => {
    x = truncate(x);
    list.appendChild(listItem(renderTruncated(x.prompt)));
    list.appendChild(sublist(x.answers.map(x => renderTruncated(x.value))));

    console.log(i, ': ', x);
});

// arr.forEach((x, i, a) => {
//     const q = new Question(x);
//     const fields = [
//         q.prompt.substring(0, 120),
//         ...q.answers.map(x => x.value.trim().substring(0, 75)),
//         60,
//         q.answers.findIndex(x => x.correct) + 1
//     ];

//     const tds = fields.map(x => tableData(x));
//     const tr = tableRow(tds);
