...
{
if (isFModeLayout() && isTouchKeyboard() && obj && ptCommonObj2.isEdibleField(obj)) {
 obj.scrollIntoView(); return false;}
...
setOpaq:function(elID, opacity) {
 ...
 if (typeof oWin == 'undefined') return; var nMaxMaskHeight = 10266;  var popHeight = nMaxMaskHeight; if (typeof id == 'undefined') id = 'pt_modalMask'; this.popMask = oWin.document.getElementById(id); if (sStyle) 
...
PT_Dialog.prototype = {
 ...
 this.processOptions(options, modObj, url);  this.maskHeader(this.idCount - 1, oParentWin, modObj.sMaskStyle, modObj.bAutoClose); var role = " role='dialog' "; if (modObj.bMsg) 
 role = " role='alertdialog' aria-describedby='alertmsg' "; else if (modObj.bCal)
 ...
 this.checkRemoveModeless(false);  var modObj = MTop().document.getElementById(MTop().PTMOD_ + id); if (!modObj) return true;  if (isFModeLayout() && isAccessibleLayout() || modObj.bViewAtt) unhidePtWrapper(); if (modObj.bDivPopup) { 
 ...
 this.checkRemoveModeless(false); for (var id = MTop().modId; id > -1; id--) {
 ...
 var mtopDocScrollTop = MTop().document.documentElement.scrollTop || MTop().document.body.scrollTop; if (isFModeLayout() && isAccessibleLayout())
 mtopDocScrollTop = 0; if (h != maxH)
...
positionPopup: function (modObj, otb, id) {
 if (!isFModeLayout() || !modObj.bPopup || modObj.bFullScreen || modObj.bCenter) return;  var oBody = document.getElementsByTagName("BODY")[0]; oBody.style.overflow = "hidden"; var oframe = MTop().document.getElementById(MTop().PTMODFRAME_ + id); if (oframe) {
 ...
 nLeft = nParentLeft; if (nLeft < 0) nLeft = nParentRight - otb.clientWidth; if ((nLeft + otb.clientWidth) > fullWidth) nLeft = fullWidth - otb.clientWidth -4; if (nLeft < 10) nLeft = 10; var nRight = fullWidth - (nLeft + otb.clientWidth); otb.style.left = nLeft + 'px'; if (!modObj.bTypeAhead) otb.style.right = nRight + 'px'; if (modObj.bTypeAhead || oframe) {
...
maskHeader: function (id0, oParentWin, sMaskStyle, bAutoClose, bClose) {
 ...
 bAutoClose = false; var modObj = null; var oheader = null; var oClose = null; var oframe = null; var idThis = -1; for (var id = id0; id > -1 && !oheader; id--) {
 var modObj = MTop().document.getElementById(MTop().PTMOD_ + id); if (!modObj) continue; oheader = MTop().document.getElementById("ptModHeader_" + id); oClose = MTop().document.getElementById("ptModCloseLnk_" + id); oframe = MTop().document.getElementById(MTop().PTMODFRAME_ + id); if (oheader)
 idThis = id; }
 

 if (!oheader || !oframe) return; var pWin = modObj.oParentWin; if (!pWin) return; var oMask = pWin.document.getElementById("pt_modalMask"); if (!oMask) return; if (bClose) {
...
{
 var modObj = top.document.getElementById(top.PTMOD_ + top.modId); var oWin = getLastModalWin(); if (modObj && modObj.bPIA && oWin && typeof oWin.bLocalModal != "undefined")
...
function getptBaseURIFromUrl(sUrl)
{
 ...
 {
 ptBaseURI = strUrl; var addressLoc = String(ptBaseURI).match(/(.)*\/ps(c|p)\/([^\/]*)?\/?([^\/]*)?\/?([^\/]*)?\//); if (addressLoc)
 ptBaseURI = addressLoc[0].replace('\/psp\/','\/psc\/'); else 
 ptBaseURI = ""; }
 else {
...
function UpdateHistory(label, keyData, userData, pageName, valid, userQueryString, nPost, bReturnToLastPage)
 ...
 var PgltAreaFrame = top.document.getElementById('ptalPgltAreaFrame'); if ( PgltAreaFrame && testRec.url == PgltAreaFrame.baseURI) { histRec.valid = 0; }
...
function DoBackService(bForm) {
 ...
 history.pop(); }
 history.save();  if (backRec && backRec.isWorkCenter) {
 top.location = sUrl; } else if (typeof backRec != 'undefind' && backRec && backRec.isClassic) {
...
function IsPageChanged()
{
 ...
 bChanged = checkFrameChanged(window); var agTarget = typeof document.getElementById("ICAGTarget") != 'undefined' && document.getElementById("ICAGTarget"); var agChanged = agTarget ? (typeof document.getElementById("ICUOW") != 'undefined' && document.getElementById("ICUOW") != null && document.getElementById("ICUOW").value != "N" ? true : false) : false; if (bChanged || agChanged) { bReturn = true; }
...
var backNavigation = {
 ...
 setTimeout(function(){cookieObject.setCookie(backNavigation.psback, backNavigation.setCookieValue(url, histRec.label, backNavigation.origin, getLayout()), backNavigation.expireDuration)}, 0);  } else { 
 setTimeout(function(){cookieObject.setCookie(backNavigation.psback, backNavigation.setCookieValue(top.location.href, setLabel(), backNavigation.origin, getLayout()), backNavigation.expireDuration)}, 0); }
 }
 ...
 cookieObject.setCookie(this.psback, this.setCookieValue(psBack.url, inLabel, psBack.origin, psBack.layout), this.expireDuration); }
 } catch(e) {}
 },
...
setAriaLabel : function(inObj, histRecLabel, histRec, histData) {
 var szType = "Back"; if (typeof bGuidedAG != "undefined" && bGuidedAG) { szType = "Exit"; }
