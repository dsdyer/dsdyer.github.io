...
document.body.onload = function() {
  ...
  window.addEventListener("message", function(e) { // This is neat! When the page gets an HTTP-POST message (which comes from
    window.opener.postMessage(true, '*');          // the search page if the bookmarklet was used), we assume the message
    tableForBuddies(e.data, undefined);            // event contains the data from a valid class search, and post a reply.
    dayChooser.style.display = 'block';            // The classFinder script is listening for the reply to know when to stop
  }, false);                                       // sending data from the search. Currently, there's no check that the
                                                   // message actually came from a class search or that the data is valid,
                                                   // hence the "This thing tends to explode" warning.
